...
export class FanSystem {
  ...
  activateFan(): boolean {
    ...
    this.config.onAddFloatingText?.(this.config.canvasWidth / 2, this.config.canvasHeight * 0.3, '强力风扇启动!', '#a78bfa');
  ...
  updateFan(deltaTime: number, roaches: Roach[]): FanState {
    ...
    if (fan.timer <= 0) {
      ...
      this.config.onAddFloatingText?.(this.config.canvasWidth / 2, this.config.canvasHeight * 0.3, '风扇停止', '#9ca3af');
      for (const r of roaches) {
        r.fanSlowTimer = 0;
        r.fanSlowFactor = 0;
        r.fanPushY = 0;
